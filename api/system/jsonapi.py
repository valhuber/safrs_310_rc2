...
class SAFRSRestAPI(Resource):
    ...
    def get(self, **kwargs):
        ...
        result_make_response = make_response(result_json)
    ...
    def _patch_instance(self, data, id=None):
        ...
        activate_checksum_patch = False  # True will fail cases of altering null values
        if not activate_checksum_patch:
            pass # should fail optLock, since checksum  (row) != checksum (updated row)
        else:
            if hasattr(instance, "CheckSum"):  # val-db
                if "CheckSum" in attributes:
                    print("valdb - don't reset Checksum - client-supplied")
                else:
                    print("valdb - reset Checksum to alert opt_locking_patch")
                    setattr(instance, "CheckSum", "!opt_locking_is_patch")
